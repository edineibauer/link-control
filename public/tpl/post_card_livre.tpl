<article class="col card color-white {$class}" {$attr} {($id!=="")?'id="' + $id + '"':''}>
    ...
    <a href="{$href}">
        ...
        <header class="col display-container">
            <img src="{$src}" class="col {$srcClass}" alt="{$alt}" title="{$title}" style="height: auto; {$srcStyle}">
            <h1 class="display-bottomleft padding-medium font-xlarge color-text-white text-shadow {$titleClass}">{$title}</h1>
        </header>
        {if $href != ""}
    </a>
    {/if}
    <div class="col padding-medium padding-24 font-light overflow-hidden {$contentClass}" style="{$contentStyle}">
        {$content}
    </div>
    {if $href != ""}
